...
const validarFormulario = (e) => {
	switch (e.target.name) {
		...
		break;
        case "cod":
            validarCampo(expresiones.fecha, e.target, 'fe');
	}
}
...
async function save(e){
    ...
    try {
        const url = `${base_url}/Vacunas/save`;
        ...
        if (result.status) {
            ...
            formRegister.reset();
            setTimeout(function(){
                window.location.href = `${base_url}/vacunas`;        
            },2500);           
        } else {
            new Noty({
                type: 'error',
...
async function edit(e){
    ...
    try {
        const url = `${base_url}/Vacunas/edit`;
    ...
    var n = new Noty({
        ...
        buttons: [
          Noty.button('YES', 'btn btn-success', async function () {
            ...
            try {
                const url = `${base_url}/Vacunas/delete`;
                const respuesta = await fetch(url, {
                    method: 'POST',
                    body: datos,
                })
